...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
   ...
   // NSLog(@"%@", [[self.newsData objectAtIndex:indexPath.row]objectForKey:NEWS_DATE]);
    ...
    NSString *className = @"";
    if([[[self.newsData objectAtIndex:indexPath.row] objectForKey:@"classID"] isEqualToString:@"0"])
        className = @"All Classes";
    else
        className = [self.mainUserData getClassName:[[self.newsData objectAtIndex:indexPath.row]objectForKey:@"classID"]];
    cell.detailTextLabel.text = [NSString stringWithFormat:@"%@ %@, %@ - %@", [HelperMethods getMonthWithInt:[startTimeArray[1] integerValue]shortName:NO], startTimeArray[2], startTimeArray[0], className];
